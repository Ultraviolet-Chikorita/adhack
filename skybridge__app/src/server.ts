...
async function loadSnapshot(source: string, demo?: unknown): Promise<GuardrailSnapshot> {
  try {
    const [reportRaw, detail] = await Promise.all([
      apiJson<unknown>("/policy/report"),
      apiJson<unknown>("/state/detail"),
    ]);
    const report = policyReportSchema.parse(reportRaw);

    return guardrailSnapshotSchema.parse({
      apiOnline: true,
      apiBase: API_BASE,
...
function textSummary(snapshot: GuardrailSnapshot): string {
  ...
  if (!report) {
    return "GuardrailBidder policy report unavailable.";
  }

  const flags = report.flags ?? {};
  return [
    `GuardrailBidder risk score ${policyRiskScore(report)}.`,
  ...
  .registerTool(
    {
      ...
      description:
        "Open an interactive GuardrailBidder dashboard view with policy flags, approvals, trace events, and sponsor integration status.",
      inputSchema: {},
      annotations: {
      ...
      view: {
        ...sharedViewConfig,
        component: "guardrail-console" as const,
  ...
  .registerTool(
    {
      ...
      description:
        "Run the seeded GuardrailBidder scenario: low intent skip, safe bid, spend spike escalation, off-brand creative block, Tavily claim check, and waste auto-pause.",
      inputSchema: {},
      annotations: {
      ...
      view: {
        ...sharedViewConfig,
        component: "guardrail-demo" as const,
        description: "GuardrailBidder seeded governance demo",
      },
    },
    async () => {
      const demo = await apiJson<unknown>("/demo/run", { method: "POST", body: "{}" });
  ...
  .registerTool(
    {
      ...
      description:
        "Score a conversational prompt, evaluate the bid, and show whether the agent can commit spend or must escalate.",
      inputSchema: {
      ...
      view: {
        ...sharedViewConfig,
        component: "bid-decision-panel" as const,
        description: "GuardrailBidder bid decision review",
      },
    },
    async ({ prompt, placement_id }) => {
      const result = await apiJson<unknown>("/bid/evaluate", {
        method: "POST",
  ...
  .registerTool(
    {
      ...
      description:
        "Judge proposed ad creative with the LLM-first safety judge, hard brand policy blocks, and Tavily-backed claim grounding.",
      inputSchema: {
      ...
      view: {
        ...sharedViewConfig,
        component: "creative-decision-panel" as const,
        description: "GuardrailBidder creative safety decision review",
      },
    },
    async ({ headline, body, claim }) => {
      const result = await apiJson<unknown>("/creative/judge-inline", {
        method: "POST",
  ...
  .registerTool(
    {
      ...
      description:
        "Upsert placement metrics, evaluate ROAS/CPA waste policy, and auto-pause clear losers with traceable rationale.",
      inputSchema: {
      ...
      view: {
        ...sharedViewConfig,
        component: "waste-decision-panel" as const,
    ...
    async ({ placement_id, impressions, clicks, spend, revenue }) => {
      ...
      await apiJson<unknown>("/placements/upsert", {
        method: "POST",
        body: JSON.stringify({
          placement_id: id,
          impressions,
      ...
      const result = await apiJson<unknown>(`/placements/evaluate-waste/${id}`, {
        method: "POST",
        body: "{}",
      });
...
server.express.get("/health", async (_req: any, res: any) => {
  try {
    ...
    res.json({ status: "ok", apiBase: API_BASE, apiOnline: true });
  } catch (error) {
    res.status(503).json({
      status: "degraded",
      apiBase: API_BASE,
